...
_VISCA_send_packet_with_reply(VISCAInterface_t *iface, VISCACamera_t *camera, VISCAPacket_t *packet)
{
  if (_VISCA_send_packet(iface,camera,packet)!=VISCA_SUCCESS){
	_RPTF0(_CRT_WARN,"VISCA_send_packet failed!\n");
    return VISCA_FAILURE;
  }

  if (_VISCA_get_reply(iface,camera)!=VISCA_SUCCESS){
	_RPTF0(_CRT_WARN,"VISCA_get_reply failed!\n");
	return VISCA_FAILURE;
  }
...
VISCA_API uint32_t
VISCA_set_address(VISCAInterface_t *iface)
{
  ...
  if (_VISCA_get_reply(iface, &camera)!=VISCA_SUCCESS)
	...
	{
	  iface->camera_num=iface->ibuf[iface->bytes-2]-1;
	  if ((iface->camera_num==0)||(iface->camera_num>7))
	    return VISCA_FAILURE;
	  else
	    return VISCA_SUCCESS;
	}
